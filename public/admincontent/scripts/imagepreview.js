    ...
    $('#banner-images-input').change(function(){
        ...
        if(/\/admin\/brands\/[A-Za-z0-9_]/i.test(fileName)){
          extraPathName = fileName.replace('/admin/brands/', '')
            ...
            extraPathName = fileName.replace('/admin/product/', '')
        }


        if(fileName == '/admin/product/create'){
            noOfAllowedImages = 5     
        }else if(fileName == `/admin/products/${extraPathName}`){
            noOfAllowedImages = 5  
        }else if(fileName == '/banner.html'){
            noOfAllowedImages = 3  
        }else if(fileName == '/admin/storefront'){
            noOfAllowedImages = 1  
        }else if(fileName == `/admin/brands/${extraPathName}`){
            noOfAllowedImages = 1  
        }else if(fileName == '/admin/collection/create'){
            noOfAllowedImages = 1  
        ...
        if(imageInput.files.length <= noOfAllowedImages){
            ...
            while(i < imageInput.files.length){
                ...
                $("#image-preview-container").clone().attr('class', `new-image-preview white-bg with-box-shadow`).appendTo("#image_preview_wrap")
